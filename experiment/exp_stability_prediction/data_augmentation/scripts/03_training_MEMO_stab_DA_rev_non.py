...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', default='model/weights_da_rev_non/')
    ...
    # load datasets
    train_pkl = pd.read_pickle('datasets/final/train_stab_da(ori+rev+non)_onehot_hhblits.pkl')
    ...
    x_test1, y_test1 = generate_dataset(test_pkl1)
    ...
    checkpoint = tf.keras.callbacks.ModelCheckpoint(args.save_dir + 'trained_weights_seed' + str(seed_num) + '.tf', monitor='val_mse', mode='min', #val_categorical_accuracy val_acc
    ...
    print(y_predict4)
    print(y_test4)
    np.save(path + 'y_pred_stab_da(non)_onehot_hhblits_seed' + str(seed_num) + '.npy', y_predict1)
    np.save(path + 'y_test_stab_da(non)_onehot_hhblits.npy', y_test1)
    np.save(path + 'y_pred_stab_da(ori+rev)_onehot_hhblits_seed' + str(seed_num) + '.npy', y_predict2)
    np.save(path + 'y_test_stab_da(ori+rev)_onehot_hhblits.npy', y_test2)
    np.save(path + 'y_pred_stab_da(rev)_onehot_hhblits_seed' + str(seed_num) + '.npy', y_predict3)
    np.save(path + 'y_test_stab_da(rev)_onehot_hhblits.npy', y_test3)
    np.save(path + 'y_pred_stab_mcsm(all)_onehot_hhblits_seed' + str(seed_num) + '.npy', y_predict4)
    np.save(path + 'y_test_stab_mcsm(all)_onehot_hhblits.npy', y_test4)
    np.save(path + 'y_pred_stab_mcsm(ori)_onehot_hhblits_seed' + str(seed_num) + '.npy', y_predict5)
    np.save(path + 'y_test_stab_mcsm(ori)_onehot_hhblits.npy', y_test5)
    np.save(path + 'y_pred_stab_mcsm(rev)_onehot_hhblits_seed' + str(seed_num) + '.npy', y_predict6)
    np.save(path + 'y_test_stab_mcsm(rev)_onehot_hhblits.npy', y_test6)