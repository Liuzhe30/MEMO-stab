...
class DataCollatorForLanguageModeling:
    ...
    def mask_tokens(
        ...
        indices_random = torch.bernoulli(
            torch.full(labels.shape, fill_value=0.5)).bool() & masked_indices & ~indices_replaced
...
class ProteinSeqDataset(Dataset):
    ...
    def __getitem__(self, index):
        item = self.protein_seq["protein"][index]
...
class Seq2Vec(nn.Module):
    ...
    def forward(
        ...
        protein_embedding = torch.stack(
            [outputs.last_hidden_state[i, protein_attention_mask[i, :], :][1:-1].mean(dim=0) for i in
             range(num_batch_size)], dim=0)
#         max_length = 1024
#         protein_embedding = torch.zeros(num_batch_size, 1024, 1024)
#         for i in range(num_batch_size):
#             selected_hidden_states = outputs.last_hidden_state[i, protein_attention_mask[i, :], :][1:-1]
#             selected_hidden_states = selected_hidden_states[:max_length]
#             protein_embedding[i, :selected_hidden_states.size(0), :] = selected_hidden_states
...
def generate_PROBE_embedding(file_path: str,
    ...
    protein_embeddings = torch.cat(protein_embeddings, dim=0).numpy()
    print(protein_embeddings.shape)
    protein_embeddings = np.mean(protein_embeddings, axis=0, keepdims=True)
    print(protein_embeddings.shape)
    np.save(embedding_save_path, protein_embeddings)
...
# files = get_files_in_directory("../Proteome/keap_csv")

# for file in files:
#     file_name = file.strip(".csv").split("/")[3]
#     generate_PROBE_embedding(file, "/datapool/home/2020012199/ProteinEmbedding/KeAP/KeAP", f"/datapool/home/2020012199/ProteinEmbedding/Proteome/keap/{file_name}.npy")
file = "/datapool/home/2020012199/ProteinEmbedding/Proteome/keap_csv/Canis familiaris (dog).csv"
file_name = "Canis familiaris (dog)"
generate_PROBE_embedding(file, "/datapool/home/2020012199/ProteinEmbedding/KeAP/KeAP", f"/datapool/home/2020012199/ProteinEmbedding/Proteome/keap/{file_name}.npy")